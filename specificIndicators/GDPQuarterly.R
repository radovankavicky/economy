...
chart.title <- "Real US Gross Domestic Product (GDPC96)\n"
...
g <- ggplot(predicted)
g <- g + geom_rect(data = recessions.trim, aes(xmin = Start, xmax = End, 
...
ggsave(g, filename = "gdp.pdf", width = 9, height = 7)
ggsave(g, filename = "gdp.png", width = 7, height = 5, dpi = 100)

# cleanup

rm(horizon, frequencyParameter, startDate, endDate, series, scaleFactor,
		y.title, chart.title)
